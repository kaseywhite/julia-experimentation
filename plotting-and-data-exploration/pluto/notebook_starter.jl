...
# ╔═╡ 20141a40-b374-11ed-0bfb-c1dd7f15706a
begin
	using Plots
	using DataFrames
	using Distributions
...
# ╔═╡ 9116774f-2c68-4a0a-af81-95edb85ec8fa
begin
...
# ╔═╡ aa7b8af4-6fbd-4f80-921a-5c1fe84fd98a
function meanrunlength(seq)
	rleseq = rle(seq) 
	return mean(rleseq[2][rleseq[1] .== 1])
end

# ╔═╡ bc01d804-75ce-4d2b-9ca8-42be73287190
begin
	flips = DataFrame(flip = [], iter = []) 
sumtable = DataFrame(iter = [] , meanrun = [], length = [] ) 
	for i = 1:6
	    f = "flips"*string(i)
	    pf = eval(Meta.parse(f))
	    bflip = split(eval(Meta.parse(f)),"")
	
	    flip = ifelse.(bflip .== "H", 1, 0)
	    iter = repeat([i], length(flip)) 
	    meanrun = meanrunlength(flip)
	
	    df = DataFrame(flip = flip, iter = iter)
	    global flips = [flips;df] 
	
	    sdf = DataFrame(iter = i, meanrun = meanrun, length = length(flip))
	    global sumtable = [sumtable;sdf]
	end
end


# ╔═╡ 228a4b39-12ce-4916-a2a9-896da34509cd
function MonteCarlo(M)
    pvalues = []
    samplemeans = [] 
    for i = 1:6
        currMean, currLength = sumtable.meanrun[i], sumtable.length[i]
        meanHeadsSample = [] 
        for j = 1:M
            sampleBinomial = rand(Bernoulli(0.5), currLength)
            meanRun = meanrunlength(sampleBinomial)
            push!(meanHeadsSample, meanRun)
        end
        push!(samplemeans, mean(meanHeadsSample)) 
        push!(pvalues,mean(meanHeadsSample .<= currMean))
    end

    df = DataFrame(iter = 1:6, means = samplemeans, pvalues = pvalues)
    return df
end

# ╔═╡ 9470518b-f85d-4be6-8b53-5e83871ba628
results = MonteCarlo(1_000_000)

# ╔═╡ 529c8785-24a0-4431-a72f-bb2beef3ebfc
print(results)

# ╔═╡ 623f27b5-3e9a-4686-9fc6-5f20d3a14ff6
@df results bar(x=:iter, y=:means)

# ╔═╡ 00cc0915-9b0a-4aa9-bfe3-149b676ee392
bar(results.iter, results.pvalues)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "54b9655ccec40f54ecdf0dbf0de4cce0f5627e46"
